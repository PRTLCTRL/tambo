...
import { useTambo } from "../v1/hooks/use-tambo-v1";
...
/**
 * Props for the TamboInteractableProvider
 */
...
> = ({ autoInteractables = false, children }) => {
  ...
  return (
    <TamboInteractableContext.Provider value={value}>
      {autoInteractables && <AutoInteractablesMonitor />}
      {children}
 ...
 * generated components to the interactables list when autoInteractables is enabled.
 * @internal
 * @returns null - this component renders nothing
 */
function AutoInteractablesMonitor(): null {
  const { addInteractableComponent } = useTamboInteractable();
  const { components: registeredComponents } = useTamboRegistry();

  // Track which component IDs have already been added to prevent duplicates
  const addedComponentIdsRef = useRef<Set<string>>(new Set());

  // Subscribe to stream state changes to detect when components finish streaming
  // This needs to be imported from the v1 stream context
  // For now, let's use a simpler approach with useTambo hook
  const tambo = useTambo();
  const currentThread = tambo?.thread;
  ...
  useEffect(() => {
    ...
    for (const message of messages) {
      ...
      for (const content of message.content) {
        ...
        // Only process components that have finished streaming
        if (componentContent.streamingState !== "done") continue;

        // Skip if already added
        const componentId = componentContent.id;
        if (addedComponentIdsRef.current.has(componentId)) continue;

        // Look up the component registration
        ...
        if (!registeredComponent) {
          ...
          addedComponentIdsRef.current.add(componentId);
          continue;
        }
        ...
        try {
          ...
          console.error(
            `[AutoInteractables] Failed to add component "${componentContent.name}":`,
            error,
          );
          addedComponentIdsRef.current.add(componentId);
        }
  ...
  }, [currentThread, addInteractableComponent, registeredComponents]);
