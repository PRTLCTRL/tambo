...
This guide shows you how to make components interactable so Tambo can modify their props in response to user messages.

There are two approaches:

1. **Manual Interactables**: Wrap specific components with `withTamboInteractable` to make them updatable
2. **Automatic Interactables**: Enable all AI-generated components to be automatically interactable
...
The fastest way to enable interactables is to turn on automatic mode. This makes every component Tambo generates automatically updatable.
...
With this enabled, users can ask Tambo to update any previously generated component:

- "Update the chart to show monthly data instead"
- "Change the note title to 'Important Reminder'"
- "Make the weather card show celsius instead of fahrenheit"

This is ideal when your UI displays only the most recent components rather than a full message history.

## Manual Interactables

For more control over which components are interactable, use manual registration by wrapping specific components with `withTamboInteractable`.
