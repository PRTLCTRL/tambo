...
<Callout type="info" title="Automatic Interactables Mode">

Rather than manually wrapping components, you can enable automatic interactables mode by setting `autoAddComponentsToInteractables={true}` on `TamboProvider`. When enabled, all AI-generated components automatically become interactable without needing `withTamboInteractable`. This is useful when you want Tambo to iterate on and refine components rather than always creating new ones.

Learn more about [Automatic Interactables](/concepts/generative-interfaces/interactable-components#automatic-interactables).
