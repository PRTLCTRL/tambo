...
By default, you manually wrap specific components with `withTamboInteractable` to make them editable by Tambo. This gives you precise control over which components Tambo can interact with.

However, you can enable a mode where **all AI-generated components automatically become interactable**. This is useful when you want Tambo to update previously-generated components rather than always creating new ones.

Enable this by setting `autoAddComponentsToInteractables={true}` on the `TamboProvider`:
...
When enabled:
- Every component Tambo generates is automatically registered as interactable
- Tambo can update any previously-generated component by modifying its props
- You don't need to manually wrap components with `withTamboInteractable`

This mode works best when:
- You want a conversation where Tambo iteratively refines components
- You're building prototypes or demos where components should be editable
- Your UI shows the list of interactable components rather than a thread of messages
