...
If you want **all** AI-generated components to automatically become interactable without manually wrapping them, enable the `autoAddComponentsToInteractables` prop on `TamboProvider`:
...
When enabled, any component the AI generates in response to a message is automatically added to the interactables registry. The AI can then update these components in future messages without requiring manual registration via `withTamboInteractable`.

This is particularly useful when:
- You want the AI to maintain and update all generated components
- You're building interfaces where components should persist and be updateable
- You want to display interactables separately from message history

**Note**: Components explicitly wrapped with `withTamboInteractable` will still work as expected and take precedence over auto-added components.
