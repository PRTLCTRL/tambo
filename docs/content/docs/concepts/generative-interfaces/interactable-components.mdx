...
By default, you manually wrap components with `withTamboInteractable` to make them interactive. However, Tambo can automatically convert all AI-generated components into interactables, allowing you to update any component the AI renders.

Enable this by setting `autoAddComponentsToInteractables` on `TamboProvider`:
...
When enabled, every component the AI generates is automatically registered as an interactable. This means:

- All generated components can be updated through natural language
- You can show a list of interactable components instead of (or alongside) the message thread
- Users can ask to modify previously-generated components without manually wrapping them

This is particularly useful when building interfaces where the AI generates a dynamic workspace of components that users then manipulate and refine.
