...
If you want **all** generated components to automatically become interactable (without manually wrapping them), enable the `autoAddToInteractables` prop on `TamboProvider`:
...
When enabled, any component that Tambo generates in response to a message will automatically be registered as an interactable. This allows you to:

1. Display a list of interactable components instead of (or alongside) the message thread
2. Let users update previously-generated components through new messages
3. Build interfaces where the AI maintains a working set of components that evolve over time

**Example use case**: Instead of showing a chat history, you could display only the current interactable components. Users ask "create a note about the meeting" and see a note appear. Then they ask "change the color to blue" and the existing note updates in place.
