...
There are two ways to make components interactable with Tambo:

### Manual Interactables (Pre-placed)
...
### Automatic Interactables (Generated Components)

Sometimes you want Tambo to update previously-generated components rather than always creating new ones. Enable this by setting `autoAddComponentsToInteractables` on your `TamboProvider`:
...
When enabled:
- Any component Tambo generates in a message is automatically added to the interactables list
- Tambo can reference and update these components in subsequent messages
- Instead of creating a new weather card, Tambo can update the existing one
- Components are only added once (tracked by component ID) to avoid duplicates

This is particularly useful for:
- Building interfaces where components should persist and update across multiple interactions
- Creating dashboards where widgets should update in place
- Displaying lists of interactables rather than thread messages
