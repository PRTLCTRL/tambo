...
Sometimes you want all generated components to become interactable automatically, without pre-placing them. This allows users to update previously-generated components instead of always creating new ones.

Enable this behavior by setting `autoInteractables` on `TamboProvider`:
...
When enabled, every component Tambo generates becomes interactable automatically. This means:

- The component appears in the interactables list accessible via `useTamboInteractable()`
- Tambo can update the component's props through natural language requests
- Users can ask to "update that chart" or "change the color of the note" and Tambo will modify the existing component

This is particularly useful for applications where you want a more conversational, iterative workflow with generated UI components.
