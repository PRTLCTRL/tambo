...
Each prop that streams receives a `PropStatus` object tracking its lifecycle. For nested objects, each nested property will have its own `PropStatus` accessible as a property. For arrays, additional `completedItems` and `streamingItems` fields provide access to items based on their streaming state.
...
interface PropStatus {
  ...
  isSuccess: boolean;
  error?: Error;
  completedItems?: unknown[]; // Only present for array props
  streamingItems?: unknown[]; // Only present for array props
  [key: string]: unknown; // Nested property statuses for object props
}
```
...
| `completedItems`  | `unknown[]` | For array props: items that have completed streaming                     |
| `streamingItems`  | `unknown[]` | For array props: items currently streaming                               |
| `[key: string]`   | `unknown`   | For object props: nested property statuses (each is also a `PropStatus`) |

**Usage for nested objects:**
...
// Render only completed items
propStatus.items?.completedItems?.map((item) => <div>{item.title}</div>);
```
