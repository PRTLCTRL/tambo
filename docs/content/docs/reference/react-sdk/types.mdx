...
Per-prop streaming status, returned by `useTamboStreamStatus()`. For nested objects, contains nested `PropStatus` objects. For arrays, includes `completedItems` and `streamingItems`.
...
| `completedItems`  | `unknown[]` | For array props: items that have completed streaming (all items except last during stream) |
| `streamingItems`  | `unknown[]` | For array props: items currently streaming (the last item while streaming)                 |

For nested object props, the `PropStatus` object contains additional nested `PropStatus` objects as properties, one for each nested field. For example, with `user: { name: string; email: string }`, the status would have a structure like `propStatus.user.name.isStreaming`.
