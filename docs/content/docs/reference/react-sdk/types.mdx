...
```typescript
interface ArrayPropStatus<T = unknown> extends PropStatus {
  completedItems: T[];
  streamingItems: T[];
}
```
...
| Property          | Type  | Description                                                   |
| ----------------- | ----- | ------------------------------------------------------------- |
| `completedItems`  | `T[]` | Array items that have finished streaming (empty while streaming) |
| `streamingItems`  | `T[]` | Array items currently being streamed (empty when complete)       |
...
// Access streaming items  
const streaming = propStatus.tasks?.streamingItems ?? [];
```

### NestedPropStatus

Recursive type for tracking nested object and array prop statuses.

```typescript
type NestedPropStatus<T = unknown> = T extends Array<infer U>
  ? ArrayPropStatus<U>
  : T extends object
...
This type automatically provides:
- `ArrayPropStatus` for array props
- Nested `PropStatus` objects for object props
- Regular `PropStatus` for primitive props
...
// Access array status
propStatus.tags?.completedItems
propStatus.tags?.streamingItems
```
