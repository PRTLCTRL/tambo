...
Extended streaming status for array props, includes item-level tracking.
...
| `streamingItems`  | `unknown[]` | Array items that are currently streaming         |
...
function TodoList(props: TodoListProps) {
  ...
  // Show only completed todos during streaming
  const todos = propStatus.todos?.completedItems ?? [];
...
Type helper for accessing streaming status of nested properties and arrays.
...
function Profile(props: UserProfile) {
  ...
  if (propStatus.user?.address?.city?.isSuccess) {
    return <span>{props.user.address.city}</span>;
  }
}
