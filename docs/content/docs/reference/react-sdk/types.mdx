...
| `streamingItems` | `unknown[]` | For array props: items currently streaming (incomplete)      |

#### Nested Object Access

For nested objects, access child property status through the parent `PropStatus`:
...
const isNameStreaming = userStatus.name.isStreaming;
```

#### Array Item Access

For array props, use `completedItems` and `streamingItems` to display only finished items:
...
// Display only completed items during streaming
const itemsToDisplay = propStatus.items?.completedItems || [];
```
