...
interface PropStatus {
  ...
  [key: string]: boolean | Error | unknown[] | PropStatus | undefined;
}
```
...
| `completedItems` | `unknown[]` | (Array props only) Items that have completed streaming                               |
| `streamingItems` | `unknown[]` | (Array props only) Items currently streaming                                         |
| `[key: string]`  | `PropStatus`| (Object props only) Nested property statuses accessed via `propStatus.parent?.child` |

#### Nested Objects

When a prop is an object, `PropStatus` includes nested property statuses:
...
propStatus.user?.email?.isPending;
```

#### Arrays

When a prop is an array, `PropStatus` includes `completedItems` and `streamingItems`:

```tsx
const { propStatus } = useTamboStreamStatus<{ items: string[] }>();

// Access array-specific fields
