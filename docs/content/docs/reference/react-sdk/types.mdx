...
interface PropStatus {
  ...
  isSuccess: boolean;
  error?: Error;
  completedItems?: unknown[]; // For arrays only
  streamingItems?: unknown[]; // For arrays only
}
...
| `[key: string]`  | `NestedPropStatus` | For object props: nested property status (recursive) |

**Nested Objects:** Each nested property has its own `PropStatus` accessible via dot notation (e.g., `propStatus.user?.name?.isStreaming`).

**Arrays:** Array props include `completedItems` and `streamingItems` fields to help you display only the items that have finished streaming, which is useful for progressive rendering.
