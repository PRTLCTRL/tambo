...
| `completedItems` | `unknown[]` | For arrays: items that have finished streaming     |
| `streamingItems` | `unknown[]` | For arrays: items currently being streamed         |

### NestedPropStatus

Type alias for recursive prop status tracking. For nested objects and arrays, `propStatus` returns a `NestedPropStatus` that mirrors the structure of your props:

```typescript
type NestedPropStatus<T> =
  T extends Array<infer U> ? PropStatus & {
    completedItems: U[];
    streamingItems: U[];
  }
  : T extends object ? PropStatus & {
    [K in keyof T]?: NestedPropStatus<T[K]>;
  }
  : PropStatus;
```

**For arrays**: Extends `PropStatus` with typed `completedItems` and `streamingItems`

**For objects**: Extends `PropStatus` with nested properties that each have their own status

**For primitives**: Same as `PropStatus`
...
// propStatus.recipes has type:
// PropStatus & {
//   completedItems: { id: string; title: string }[];
//   streamingItems: { id: string; title: string }[];
// }
```
