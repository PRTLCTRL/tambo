...
For **nested object props**, each nested property has its own `PropStatus` accessible via the property key.

For **array props**, includes `completedItems` (items that finished streaming) and `streamingItems` (items currently being streamed).
...
interface PropStatus {
  ...
  isSuccess: boolean;
  error?: Error;
  completedItems?: unknown[]; // For array props
  streamingItems?: unknown[]; // For array props
...
| `completedItems`  | `unknown[]` | (Array props only) Items that have finished streaming       |
| `streamingItems`  | `unknown[]` | (Array props only) Items currently being streamed           |
| `[key: string]`   | `unknown`   | (Object props only) Nested PropStatus for nested properties |
