...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Supports nested objects and includes array-specific properties.
...
| `completedItems`  | `unknown[]` | For array props: items that have completed streaming. Only present for array props |
| `streamingItems`  | `unknown[]` | For array props: items currently being streamed. Only present for array props      |

**Nested Objects**: For object properties, the `PropStatus` contains nested status objects for each child property.

**Arrays**: For array properties, `completedItems` contains items that have finished streaming, and `streamingItems` contains items still being streamed.
