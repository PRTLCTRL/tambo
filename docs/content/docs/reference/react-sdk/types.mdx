...
interface PropStatus {
  ...
  [nestedKey: string]: PropStatus | boolean | Error | unknown[] | undefined;
}
```
...
| `completedItems` | `unknown[]` | (Arrays only) Items that have completed streaming            |
| `streamingItems` | `unknown[]` | (Arrays only) Items currently being streamed (incomplete)    |
| `[nestedKey]`    | `PropStatus`| (Objects only) Nested property status for child fields       |
...
// Access array tracking:
propStatus.items?.completedItems // Items that finished streaming
propStatus.items?.streamingItems  // Items still streaming
```
