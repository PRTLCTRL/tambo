...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Supports nested objects and arrays with granular tracking.
...
| `completedItems` | `unknown[]` | For array props: items that have finished streaming (only present for arrays)|
| `streamingItems` | `unknown[]` | For array props: items currently streaming (only present for arrays)         |

#### Nested Object Support

For props that are objects, `PropStatus` includes nested status for each property:

```typescript
// Example: user prop with nested name and email
propStatus.user?.name?.isStreaming; // Status of user.name
propStatus.user?.email?.isSuccess;  // Status of user.email
```

#### Array Support

For array props, use `completedItems` and `streamingItems` to display partial results:

```typescript
// Example: tasks array prop
propStatus.tasks?.completedItems?.map(task => <TaskCard {...task} />)
propStatus.tasks?.streamingItems?.map(task => <TaskCard {...task} isStreaming />)
```
