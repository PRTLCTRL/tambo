...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Supports nested object tracking and array item tracking.
...
For nested objects, `PropStatus` includes nested properties that mirror the prop structure. Access nested status via dot notation (e.g., `propStatus.user?.name?.isStreaming`).
