...
Per-prop streaming status, returned by `useTamboStreamStatus()`. For nested objects, contains additional nested `PropStatus` fields. For arrays, includes `completedItems` and `streamingItems`.
...
For nested objects, `PropStatus` recursively contains status for nested fields. Access using dot notation:

```typescript
propStatus.user?.name?.isStreaming
propStatus.config?.database?.host?.isSuccess
```
