...
For nested object props, the PropStatus object contains nested PropStatus objects matching the prop structure. For array props, it includes `completedItems` and `streamingItems` arrays.
...
interface PropStatus {
  ...
  isSuccess: boolean;
  error?: Error;
  completedItems?: unknown[]; // For array props only
  streamingItems?: unknown[]; // For array props only
  [key: string]: unknown; // For nested object props
}
```
...
| `completedItems`  | `unknown[]` | For array props: items that have finished streaming                   |
| `streamingItems`  | `unknown[]` | For array props: items currently being streamed (partial/in-progress) |
| `[key: string]`   | `unknown`   | For nested object props: nested PropStatus for each property          |
...
// Access completed/streaming items:
