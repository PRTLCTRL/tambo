...
| `[key: string]`   | `unknown`   | For objects: nested PropStatus for each field             |

**Nested Objects**: For object props, `PropStatus` includes nested status for each field. Access like `propStatus.user.name.isStreaming`.

**Arrays**: For array props, `completedItems` contains items that finished streaming and `streamingItems` contains items still in progress.
