...
| `streamingItems` | `unknown[]` | For array props: items currently being streamed (only present for arrays)       |

For nested objects, the `PropStatus` object contains additional properties matching the nested structure. Access nested status via dot notation: `propStatus.parent?.child?.isStreaming`.

For arrays, use `completedItems` to render items that have finished streaming while the rest continue to arrive.
