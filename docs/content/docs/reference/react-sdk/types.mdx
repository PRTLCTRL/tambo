...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Supports nested objects and arrays with specialized tracking.
...
interface PropStatus {
  ...
  // For nested object props: nested PropStatus objects
  [key: string]: unknown;
}
```
...
| `completedItems` | `unknown[]` | (Arrays only) Items that have finished streaming                         |
| `streamingItems` | `unknown[]` | (Arrays only) Items currently streaming                                  |
| `[key: string]`  | `unknown`   | (Objects only) Nested PropStatus objects for nested properties           |

#### Nested Objects

For nested object props, access nested status using dot notation:
...
For array props, use `completedItems` to display only fully-streamed items:
...
const displayItems = propStatus.items?.completedItems || [];

// Check if more items are streaming
const hasMoreItems = (propStatus.items?.streamingItems?.length ?? 0) > 0;
```
