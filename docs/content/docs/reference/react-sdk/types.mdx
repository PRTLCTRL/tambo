...
For nested objects, contains nested `PropStatus` structures. For arrays, includes `completedItems` and `streamingItems`.
...
| `completedItems`  | `unknown[]` | For arrays: items that have completed streaming              |
| `streamingItems`  | `unknown[]` | For arrays: items currently being streamed                   |
| `[key: string]`   | `PropStatus | ...` | For objects: nested status for child properties   |
...
if (propStatus.user?.name?.isStreaming) {
  // The user.name property is currently streaming
}

if (propStatus.user?.email?.isSuccess) {
  // The user.email property has finished streaming
}
```
...
return (
  <>
    ...
    {streaming.map((task) => (
      <TaskCard key={task.id} {...task} isLoading />
    ))}
  </>
);
```
