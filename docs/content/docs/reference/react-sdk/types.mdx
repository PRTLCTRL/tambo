...
For nested objects, the status object mirrors the prop structure, allowing access like `propStatus.user?.name?.isStreaming`.
...
interface PropStatus {
  ...
  [key: string]: unknown; // Nested PropStatus for object props
}
```
...
| `streamingItems` | `unknown[]` | For array props: items currently being streamed (not yet complete) |
| `[key: string]`  | `unknown`   | For object props: nested PropStatus for each property            |
...
if (propStatus.user?.name?.isStreaming) {
  // user.name is currently streaming
}
```
...
// Display completed items while streaming continues
return (
  <ul>
