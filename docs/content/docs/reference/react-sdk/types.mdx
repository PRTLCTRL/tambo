...
For nested objects, `PropStatus` contains nested status objects for each child property.
For arrays, `PropStatus` includes `completedItems` and `streamingItems` arrays.
...
| `streamingItems`  | `unknown[]` | For arrays: items that are currently streaming                       |
| `[nestedKey]`     | `unknown`   | For objects: nested `PropStatus` objects for each child property     |
