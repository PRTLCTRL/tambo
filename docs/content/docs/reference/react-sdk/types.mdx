...
| `streamingItems` | `unknown[]` | **(Arrays only)** Items currently being streamed                   |

For nested objects, `PropStatus` extends to include nested properties with their own status. For example, a prop `user: { name: string; email: string }` would have `propStatus.user.name` and `propStatus.user.email` accessible.
