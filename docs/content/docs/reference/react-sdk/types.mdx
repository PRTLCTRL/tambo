...
Per-prop streaming status, returned by `useTamboStreamStatus()`. For nested objects, each nested field has its own `PropStatus`. For arrays, includes `completedItems` and `streamingItems`.
...
interface PropStatus {
  ...
  [key: string]: PropStatus | boolean | Error | unknown[] | undefined; // For nested objects
}
```
...
| `streamingItems`  | `unknown[]` | For array props: items currently being streamed                                          |
| `[key]`           | `PropStatus`| For object props: nested `PropStatus` for each property (access via `propStatus.parent.child`) |
