...
Per-prop streaming status, returned by `useTamboStreamStatus()`. For nested objects, each nested property has its own `PropStatus`. For arrays, `completedItems` and `streamingItems` provide access to items by streaming state.
...
interface PropStatus {
  ...
  isSuccess: boolean;
  error?: Error;
  completedItems?: unknown[]; // Only for array props
  streamingItems?: unknown[]; // Only for array props
}
```
...
| `streamingItems`  | `unknown[]`  | (Array props only) Items currently streaming (partial content) |
