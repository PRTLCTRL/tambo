...
For nested objects, the `PropStatus` recursively contains status for nested properties:

```typescript
// For props like: { user: { name: string, email: string } }
propStatus.user?.name?.isStreaming; // Access nested property status
propStatus.user?.email?.isSuccess; // Check if nested field is complete
```

#### Arrays

For array props, use `completedItems` and `streamingItems` to display only completed items during streaming:

```typescript
// For props like: { items: string[] }
