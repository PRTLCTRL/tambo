...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Includes additional fields for array props to track completed and streaming items.
...
| `streamingItems`  | `unknown[]` | For array props: items currently being streamed (usually last)  |

**Nested Objects**: For object props, `PropStatus` also contains nested `PropStatus` objects for each property, allowing you to track the streaming state of deeply nested fields. Access them by casting to an intersection type like `PropStatus & { fieldName?: PropStatus }`.

**Array Props**: The `completedItems` and `streamingItems` fields are only populated for array props. During streaming, all items except the last are in `completedItems`, and the last item is in `streamingItems`. When streaming completes, all items move to `completedItems`.
