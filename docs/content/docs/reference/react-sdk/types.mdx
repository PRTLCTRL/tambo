...
**Nested Object Support**: For object props, each nested property has its own `PropStatus` accessible via dot notation (e.g., `propStatus.user.name.isStreaming`).

**Array Support**: For array props, use `completedItems` to access items that have finished streaming, and `streamingItems` for items currently streaming.
