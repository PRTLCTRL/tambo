...
For nested objects, `PropStatus` includes nested status fields for each property. For arrays, `PropStatus` includes `completedItems` and `streamingItems` arrays.
...
| `streamingItems`  | `unknown[]` | (Arrays only) Items currently streaming (typically just the last item)     |
| `[key: string]`   | `PropStatus`| (Objects only) Nested PropStatus for accessing nested property status      |
...
const nameIsStreaming = (propStatus.user as any)?.name?.isStreaming;
```
...
const streaming = propStatus.items?.streamingItems as string[] ?? [];
```
