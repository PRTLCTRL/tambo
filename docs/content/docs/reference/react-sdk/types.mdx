...
interface PropStatus {
  ...
  // For nested objects: nested status by property name
  [key: string]: PropStatus | boolean | Error | unknown[] | undefined;
  
  // For arrays: completed and streaming items
...
| `streamingItems` | `unknown[]` | (Arrays only) Items currently streaming or pending                          |
| `[key]`          | `PropStatus`| (Nested objects only) Status for nested property, accessed via dot notation |
...
propStatus.user?.name?.isStreaming; // boolean
propStatus.user?.email?.isSuccess;  // boolean
```
