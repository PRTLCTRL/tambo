...
Per-prop streaming status, returned by `useTamboStreamStatus()`. Supports flat values, nested objects, and arrays.

```typescript
type PropStatus = BasePropStatus | ArrayPropStatus | NestedPropStatus;
...
interface ArrayPropStatus extends BasePropStatus {
  completedItems: unknown[];
  streamingItems: unknown[];
}

type NestedPropStatus = BasePropStatus & {
  [key: string]: PropStatus;
};
```

**Base Properties** (present for all prop types):
...
**Array-Specific Properties** (when prop value is an array):

| Property          | Type        | Description                                      |
| ----------------- | ----------- | ------------------------------------------------ |
| `completedItems`  | `unknown[]` | Items that have finished streaming               |
| `streamingItems`  | `unknown[]` | Items currently being streamed                   |

**Nested Object Properties** (when prop value is an object):

For nested objects, the `PropStatus` recursively contains status for each nested property, allowing access like `propStatus.user?.name?.isStreaming`.
