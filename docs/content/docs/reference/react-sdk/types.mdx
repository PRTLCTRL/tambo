...
| `streamingItems`  | `unknown[]` | For array props: items currently streaming (not yet complete) |

For nested object props, each nested property also has its own `PropStatus` accessible via dot notation (e.g., `propStatus.user.name.isStreaming`).
