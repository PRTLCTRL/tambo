...
| `streamingItems` | `unknown[]` | For array props: items currently streaming or pending                    |
| `[key]`          | `unknown`   | For object props: nested property statuses (each is also a `PropStatus`) |

**Nested Objects**: For props with nested objects, additional keys represent nested property statuses. Access them via chained property access: `propStatus.user?.name?.isStreaming`.

**Arrays**: Array props include `completedItems` (all items except the last during streaming) and `streamingItems` (the item currently being streamed). Once streaming completes, all items are in `completedItems`.
