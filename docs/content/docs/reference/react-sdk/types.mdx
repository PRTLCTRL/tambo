...
| `completedItems` | `unknown[]` | Array of completed items (for array props only)                 |
| `streamingItems` | `unknown[]` | Array of currently streaming items (for array props only)       |

**Nested Objects**: For props that are objects, `PropStatus` includes nested status properties matching the object structure.

**Arrays**: For props that are arrays, `PropStatus` includes `completedItems` and `streamingItems` to access items by their streaming state.
