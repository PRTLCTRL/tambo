...
### Nested Object Status Tracking

For props with nested objects, `propStatus` mirrors the structure with nested status objects:
...
function UserCard({ user }: UserCardProps) {
  ...
  return (
    <div>
      ...
      <p className={propStatus.user?.address?.city?.isStreaming ? "animate-pulse" : ""}>
        {user?.address?.city}
      </p>
    </div>
  );
...
function StreamingList({ items }: ListProps) {
  ...
  return (
    <div>
      <p>Completed: {completedCount}, Streaming: {streamingCount}</p>
      <ul>
        {propStatus.items?.completedItems?.map((item: any) => (
          <li key={item.id}>{item.text}</li>
        ))}
        {propStatus.items?.streamingItems?.map((item: any) => (
          <li key={item.id} className="animate-pulse">
            {item.text}
          </li>
        ))}
      </ul>
...
<Callout type="info">
  When streaming is done, all items are in `completedItems`. When streaming is active, all items are in `streamingItems`. When pending, both arrays are empty.
</Callout>
