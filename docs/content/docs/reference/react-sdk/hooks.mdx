...
Track streaming status of nested object properties by accessing them through the status hierarchy:
...
function UserCard(props: NestedProps) {
  const { propStatus } = useTamboStreamStatus<NestedProps>();
  ...
  return (
    <div>
      ...
      {propStatus.user?.email?.isPending && <span>Loading email...</span>}
    </div>
  );
}
```
...
For array props, access `completedItems` and `streamingItems` to render items incrementally:
