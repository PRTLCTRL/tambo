...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      <h3
        className={propStatus.user?.name?.isStreaming ? "animate-pulse" : ""}
      >
        {props.user.name}
      </h3>
...
function RecipeList(props: RecipeListProps) {
  ...
  return (
    <div>
      <h2>Recipes</h2>
      {completedRecipes.map((recipe: any) => (
        <div key={recipe.id}>
          <h3>{recipe.title}</h3>
          <ul>
      ...
      {streamingRecipes.map((recipe: any, i: number) => (
        <div key={i} className="animate-pulse">
          <h3>{recipe.title || "Loading..."}</h3>
          <p>Streaming recipe details...</p>
        </div>
      ))}
    </div>
  );
