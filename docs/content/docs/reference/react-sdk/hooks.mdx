...
Track streaming status for components rendered via `ComponentRenderer`. Provides both overall stream status and per-prop granularity, including support for nested objects and arrays.
...
### Tracking Nested Object Properties

The hook automatically tracks streaming status for nested object properties. Access nested status using dot notation through the type system:
...
function UserCard(props: UserCardProps) {
  ...
  // Type-safe access to nested property status
  ...
  return (
    <div>
      ...
      <p className={userStatus.email?.isStreaming ? "animate-pulse" : ""}>
        {props.user.email}
      </p>
      {userStatus.profile?.bio?.isSuccess && (
...
### Tracking Array Items

For array props, the hook provides `completedItems` and `streamingItems` to display items as they arrive:
...
function RecipeList(props: RecipeListProps) {
  ...
  const recipesStatus = propStatus.recipes;

  return (
    <div>
      {/* Show completed items normally */}
      {recipesStatus?.completedItems?.map((recipe) => (
        <RecipeCard key={recipe.id} {...recipe} />
      ))}

      {/* Show streaming items with loading indicator */}
      {recipesStatus?.streamingItems?.map((recipe) => (
        <RecipeCard key={recipe.id} {...recipe} loading />
      ))}

      {/* Show loading state while waiting for first item */}
      {recipesStatus?.isPending && <Skeleton count={3} />}
    </div>
  );
}
```

This pattern is especially useful when streaming lists of items - users see completed items immediately while new items continue to arrive.
...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus<T>>>`      | Per-prop streaming status with nested support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |

For array props, `NestedPropStatus` extends `PropStatus` with:

- `completedItems`: Array of items that have finished streaming
- `streamingItems`: Array of items currently being streamed (typically the last item)

For object props, `NestedPropStatus` extends `PropStatus` with nested properties that mirror the structure of the object, each with their own status.
