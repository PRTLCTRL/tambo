...
For props with nested objects, `propStatus` includes nested property statuses:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      ...
      <p className={userStatus?.address?.city?.isStreaming ? "animate-pulse" : ""}>
...
Access nested status via chained property access: `propStatus.user?.name?.isStreaming` or `propStatus.user?.address?.city?.isSuccess`.
...
function TaskList(props: TaskListProps) {
  ...
  return (
    <div>
      ...
      <ul>
        {tasksStatus?.completedItems?.map((task) => (
      ...
      {tasksStatus?.streamingItems && tasksStatus.streamingItems.length > 0 && (
        <>
          ...
          <ul>
            {tasksStatus.streamingItems.map((task) => (
...
During streaming, `completedItems` contains all items except the last one, and `streamingItems` contains the item currently being streamed. Once streaming completes, all items move to `completedItems`.
