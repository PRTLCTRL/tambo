...
### Nested Object Streaming

Access nested property status by traversing the `propStatus` object:
...
For array props, access `completedItems` and `streamingItems` to render only completed items or show loading states:
...
function TaskList({ tasks }: TaskListProps) {
  ...
  return (
    <ul>
      {propStatus.tasks?.completedItems?.map((task, i) => (
        <li key={i}>{task}</li>
      ))}
      {propStatus.tasks?.streamingItems?.map((task, i) => (
