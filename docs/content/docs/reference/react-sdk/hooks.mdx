...
The hook supports tracking streaming status for nested object properties. Access nested property status by traversing the status object:
...
function UserCard(props: UserProps) {
  ...
  return (
    <div>
      {/* Track parent object status */}
      {propStatus.user?.isStreaming && <Spinner />}

      {/* Track nested property status */}
      <h3 className={propStatus.user?.name?.isStreaming ? "animate-pulse" : ""}>
        {props.user.name}
      </h3>

      {/* Track deeply nested property status */}
...
For array props, the hook provides `completedItems` and `streamingItems` to render partial arrays during streaming:
...
function ItemList(props: ListProps) {
  ...
  return (
    <div>
      ...
      {/* Show loading state for streaming items */}
      {propStatus.items?.streamingItems?.map((item, i) => (
        <div key={`streaming-${i}`} className="animate-pulse">
          {item.name}
        </div>
      ))}

      {/* Wait for all items before showing action */}
      {propStatus.items?.isSuccess && (
        <button>Process all {props.items.length} items</button>
      )}
    </div>
  );
}
