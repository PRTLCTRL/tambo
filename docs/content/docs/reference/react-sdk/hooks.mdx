...
When props contain nested objects, `propStatus` includes nested status objects for each level. Access nested streaming status using dot notation on the status object:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      {propStatus.user?.name?.isStreaming && <Skeleton className="h-6 w-32" />}
      {!propStatus.user?.name?.isStreaming && <h2>{props.user.name}</h2>}

      {propStatus.user?.profile?.bio?.isStreaming && (
        <Skeleton className="h-20 w-full" />
      )}
      {propStatus.user?.profile?.bio?.isSuccess && <p>{props.user.profile.bio}</p>}
    </div>
  );
}
```
...
For array props, `PropStatus` includes `completedItems` (items that finished streaming) and `streamingItems` (the current item being streamed). This lets you display completed items while the last item is still generating:
...
function ProductList(props: ProductListProps) {
  const { propStatus } = useTamboStreamStatus<ProductListProps>();

  const completedProducts = propStatus.products?.completedItems ?? [];
  const streamingProduct = propStatus.products?.streamingItems?.[0];
  ...
  return (
    <ul>
      ...
      {streamingProduct && (
        <li className="animate-pulse opacity-50">
          {streamingProduct.name} - ${streamingProduct.price}
        </li>
      )}
    </ul>
  );
...
When streaming completes, all items move to `completedItems` and `streamingItems` becomes `undefined`.
