...
Track streaming status for nested object properties:
...
function UserCard(props: UserCardProps) {
  ...
  // Access nested prop status via type assertion
  ...
  return (
    <div>
      <h2 className={userStatus.name?.isStreaming ? "pulse" : ""}>
        {props.user.name}
      </h2>
      <p className={userStatus.email?.isStreaming ? "pulse" : ""}>
        {props.user.email}
      </p>
      {userStatus.profile.avatar.isSuccess && (
...
Display only completed items from array props during streaming:
...
function ProductList(props: ListProps) {
  const { propStatus } = useTamboStreamStatus<ListProps>();

  // Show only completed items while streaming
  const displayItems =
    propStatus.items?.completedItems || props.items || [];
  ...
  return (
    <ul>
      ...
      {propStatus.items?.isStreaming && (
        <li>Loading more items...</li>
      )}
    </ul>
  );
}
...
### Mixed Nested and Array Support
...
function Dashboard(props: DashboardProps) {
  const { propStatus, streamStatus } = useTamboStreamStatus<DashboardProps>();

  const configStatus = propStatus.config as PropStatus & {
    theme: PropStatus;
    layout: PropStatus;
  };

  // Render completed widgets only
  const completedWidgets =
    (propStatus.widgets?.completedItems as DashboardProps["widgets"]) || [];

  return (
    <div>
      {configStatus.theme?.isSuccess && (
        <div className={`theme-${props.config.theme}`}>
          {completedWidgets.map((widget) => (
            <Widget key={widget.id} {...widget} />
          ))}
        </div>
      )}
      {streamStatus.isStreaming && <LoadingSpinner />}
    </div>
  );
}
```
