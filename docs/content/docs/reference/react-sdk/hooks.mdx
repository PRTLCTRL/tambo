...
Track streaming status for deeply nested object properties:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      {userStatus?.name?.isStreaming && <Spinner />}
...
Track which array items have completed streaming vs are still in progress:
  ...
  return (
    <div>
      ...
      {streamingTasks.map((task) => (
        <TaskItem key={task.id} task={task} loading />
      ))}
    </div>
  );
}
