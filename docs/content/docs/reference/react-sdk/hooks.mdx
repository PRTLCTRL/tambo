...
function UserCard(props: UserCardProps) {
  ...
  // Access nested property status
  const userStatus = propStatus.user as any;
  const nameStatus = userStatus?.name;
  const profileStatus = userStatus?.profile as any;
  const bioStatus = profileStatus?.bio;

  return (
    <div>
      <h3 className={nameStatus?.isStreaming ? "animate-pulse" : ""}>
        {props.user.name}
      </h3>
      {nameStatus?.isSuccess && <p>{props.user.email}</p>}
      {bioStatus?.isStreaming && <p>Loading bio...</p>}
      {bioStatus?.isSuccess && <p>{props.user.profile.bio}</p>}
    </div>
  );
}
```
...
Access completed and streaming items from array properties:
...
function RecipeList(props: RecipeListProps) {
  ...
  const recipesStatus = propStatus.recipes as any;
  const completedRecipes = recipesStatus?.completedItems || [];
  const streamingRecipes = recipesStatus?.streamingItems || [];
  ...
  return (
    <div>
      ...
      {streamingRecipes.map((recipe, index) => (
        <RecipeCard key={`streaming-${index}`} {...recipe} isLoading />
      ))}

      {recipesStatus?.isStreaming && (
        <p>Loading {streamingRecipes.length} more recipes...</p>
      )}
    </div>
  );
}
