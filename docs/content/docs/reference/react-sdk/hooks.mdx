...
Track streaming status of nested object properties individually:
...
function UserCard(props: UserProps) {
  ...
  return (
    <div>
      ...
      {propStatus.user?.address?.isSuccess && (
        <div>
          <p>{props.user.address.city}</p>
          <p>{props.user.address.zip}</p>
        </div>
      )}
    </div>
  );
...
For array props, access completed and streaming items separately:
...
function TaskList(props: TaskListProps) {
  ...
  return (
    <div>
      <ul>
        {propStatus.tasks?.completedItems?.map((task) => (
          <li key={task.id}>{task.title}</li>
        ))}
      </ul>
      {propStatus.tasks?.streamingItems &&
        propStatus.tasks.streamingItems.length > 0 && (
          <div className="opacity-50">
            <p>
              Streaming {propStatus.tasks.streamingItems.length} more items...
            </p>
          </div>
        )}
    </div>
...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus>>`          | Per-prop streaming status with nested structure support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
...
<Callout type="tip">
  For nested objects, access nested status via
  `propStatus.parent?.child?.isStreaming`. For arrays, use
  `propStatus.arrayField?.completedItems` to render only finished items.
</Callout>
