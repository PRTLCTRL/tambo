...
Track streaming status for components rendered via `ComponentRenderer`. Provides both overall stream status and per-prop granularity, including nested objects and arrays.
...
`useTamboStreamStatus` now tracks streaming status for nested object properties. Each nested field has its own `PropStatus` that you can access via nested property access:
...
function UserCard(props: UserCardProps) {
  ...
  // Track status of the entire user object
  const userStreaming = propStatus.user?.isStreaming;

  // Track status of nested properties
  const nameStreaming = propStatus.user?.name?.isStreaming;
  const emailPending = propStatus.user?.email?.isPending;
  const bioComplete = propStatus.user?.profile?.bio?.isSuccess;

  return (
    <div>
      <h2 className={nameStreaming ? "animate-pulse" : ""}>
        {props.user?.name}
      </h2>
      {emailPending && <Skeleton />}
      {bioComplete && <p>{props.user.profile.bio}</p>}
    </div>
  );
}
```
...
For array props, `propStatus` includes `completedItems` and `streamingItems` arrays. This lets you render completed items immediately while showing loading states for items still streaming:
...
function TaskList(props: TaskListProps) {
  ...
  return (
    <div>
      ...
      {/* Show loading for streaming items */}
      {streamingTasks.map((task, i) => (
        <TaskItem key={`streaming-${i}`} task={task} loading />
      ))}
    </div>
  );
}
