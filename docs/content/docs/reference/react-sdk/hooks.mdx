...
Track streaming status of deeply nested properties:
...
Access completed and streaming items separately:
...
function TaskList(props: TaskListProps) {
  ...
  // Only render completed items while streaming
  const completedTasks = propStatus.tasks?.completedItems ?? [];
  const isStreamingTasks = propStatus.tasks?.isStreaming ?? false;
...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus>>`          | Per-prop streaming status with nested support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus) and `NestedPropStatus` for details. |
