...
For props with nested objects, `propStatus` provides a nested structure that mirrors your prop shape:
...
function UserCard({ user }: UserCardProps) {
  ...
  return (
    <div>
      ...
      <p
        className={
          propStatus.user?.profile?.bio?.isStreaming ? "animate-pulse" : ""
        }
      >
        {user.profile.bio}
      </p>
...
For array props, `propStatus` includes `completedItems` and `streamingItems` to access items by their streaming state:
...
function TodoList({ todos }: TodoListProps) {
  ...
  // Get items that have finished streaming
  const completed = propStatus.todos?.completedItems || [];
  const streaming = propStatus.todos?.streamingItems || [];
  ...
  return (
    <div>
      ...
      {streaming.length > 0 && (
        <>
          <h3>Loading... ({streaming.length})</h3>
...
This allows you to render completed items immediately while showing loading states for items still streaming.
