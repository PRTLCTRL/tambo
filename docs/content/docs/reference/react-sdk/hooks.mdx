...
For props with nested objects, `propStatus` includes nested status fields. You can track the streaming status of individual nested properties:
...
function UserProfile(props: UserProfileProps) {
  const { propStatus } = useTamboStreamStatus<UserProfileProps>();

  return (
    <div>
      <h2 className={(propStatus.user as any)?.name?.isStreaming ? "animate-pulse" : ""}>
        {props.user.name}
      </h2>
      <p className={(propStatus.user as any)?.email?.isStreaming ? "animate-pulse" : ""}>
        {props.user.email}
      </p>
      <p className={(propStatus.user as any)?.profile?.bio?.isStreaming ? "animate-pulse" : ""}>
...
For array props, `propStatus` includes `completedItems` and `streamingItems` arrays. This allows you to display only the items that have finished streaming:
...
function TodoList(props: TodoListProps) {
  ...
  const completedTodos = (propStatus.todos?.completedItems as typeof props.todos) ?? [];
  const streamingTodos = (propStatus.todos?.streamingItems as typeof props.todos) ?? [];
        ...
        <>
          ...
          <ul>
            {streamingTodos.map(todo => (
              <li key={todo.id} className="animate-pulse">{todo.title}</li>
            ))}
          </ul>
        </>
      )}
