...
The hook supports tracking streaming status for nested object properties and array elements.
...
function UserCard(props: UserCardProps) {
  ...
  // Access nested property status by casting (TypeScript doesn't infer deep structure)
  const userStatus = propStatus.user as any;

  return (
    <div>
      {/* Show streaming indicator on individual nested fields */}
      <div className={userStatus?.name?.isStreaming ? "animate-pulse" : ""}>
        {props.user?.name}
      </div>
      
      {/* Check if nested field has completed before rendering */}
      {userStatus?.email?.isSuccess && (
        <a href={`mailto:${props.user?.email}`}>{props.user?.email}</a>
      )}

      {/* Access deeply nested properties */}
      {userStatus?.address?.city?.isSuccess && (
        <span>{props.user?.address?.city}, {props.user?.address?.country}</span>
      )}
    </div>
  );
}
...
For array props, the hook provides `completedItems` and `streamingItems` to help you display only the items that have finished streaming:
...
function TaskList(props: TaskListProps) {
  ...
  // During streaming, only show completed items
  const itemsToShow = propStatus.tasks?.isSuccess
    ? props.tasks // Show all when done
    : (propStatus.tasks?.completedItems?.map(item => item.value) ?? []); // Show completed during streaming
  ...
  return (
    <ul>
      ...
      {/* Show loading indicator for streaming items */}
      {propStatus.tasks?.streamingItems?.length > 0 && (
        <li className="animate-pulse">Loading more tasks...</li>
      )}
    </ul>
  );
}
...
The `completedItems` and `streamingItems` arrays contain objects with `{ index: number, value: unknown }` structure, allowing you to track both the position and value of each item.
