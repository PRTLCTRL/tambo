...
For components with nested object props, you can track the streaming status of individual nested properties:
...
For array props, `propStatus` includes `completedItems` and `streamingItems` fields to access items based on their streaming state:
...
function TaskList(props: TaskListProps) {
  ...
  return (
    <ul>
      ...
      {propStatus.tasks?.streamingItems?.map((task, index) => (
        <li key={`streaming-${index}`} className="opacity-50">
