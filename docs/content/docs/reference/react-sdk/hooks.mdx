...
For components with nested object props, `propStatus` contains nested status structures that mirror your prop shape:
...
For array props, `propStatus` includes `completedItems` and `streamingItems` to help you render only fully streamed items:
...
This is particularly useful when array items contain data you need to fetch or process. By waiting for items to complete streaming, you avoid making premature API calls with partial data.
