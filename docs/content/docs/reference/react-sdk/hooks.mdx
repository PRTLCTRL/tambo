...
| `propStatus`   | `Partial<Record<keyof Props, PropStatus>>`                | Per-prop streaming status with nested structure support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |

### Nested Object Support

When your component props include nested objects, `propStatus` provides streaming status for each nested property:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      {propStatus.user?.name?.isStreaming && <Skeleton />}
      <h2>{props.user.name}</h2>

      {propStatus.user?.profile?.bio?.isStreaming && <Skeleton />}
...
Access nested status using dot notation: `propStatus.user?.name?.isStreaming`

### Array Support

For array props, `propStatus` includes `completedItems` and `streamingItems` to help you render only completed items during streaming:
...
function TaskList(props: TaskListProps) {
  ...
  const isStreaming = propStatus.tasks?.isStreaming;
  ...
  return (
    <ul>
      ...
      {isStreaming && <li>Loading more...</li>}
    </ul>
  );
}
```

Array status properties:
- `completedItems`: Items that have finished streaming (empty array while streaming, full array when done)
- `streamingItems`: Items currently being streamed (populated during streaming, empty when done)
