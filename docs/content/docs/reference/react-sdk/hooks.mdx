...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus>>`          | Per-prop streaming status with nested tracking. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
...
The hook automatically tracks streaming status for nested objects. Access nested properties using dot notation:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      <h2 className={propStatus.user?.name?.isStreaming ? "opacity-50" : ""}>
        {props.user?.name}
      </h2>
      <p className={propStatus.user?.email?.isStreaming ? "opacity-50" : ""}>
        {props.user?.email}
      </p>
      {propStatus.user?.profile?.bio?.isSuccess && (
...
Each nested level has its own `PropStatus` with `isPending`, `isStreaming`, `isSuccess`, and `error` fields.

### Array Tracking with Completed Items

For array props, the status object includes `completedItems` and `streamingItems` arrays to help you display only the items that have finished streaming:
...
function TaskList(props: TaskListProps) {
  ...
  // Display only completed tasks during streaming
  const tasksToShow = propStatus.tasks?.isSuccess
    ? props.tasks
    : propStatus.tasks?.completedItems ?? [];
  ...
  return (
    <ul>
      ...
      {propStatus.tasks?.isStreaming && (
        <li className="opacity-50">Loading more tasks...</li>
      )}
    </ul>
  );
}
...
- `completedItems`: Array items that have finished streaming (available when streaming is complete)
- `streamingItems`: Array items currently being streamed (available during streaming)
- `isPending`, `isStreaming`, `isSuccess`, `error`: Status flags for the array itself

This is especially useful for rendering lists progressively as items complete, improving the user experience during long streaming operations.
