...
Access streaming status for nested object properties by following the object path:
...
function UserCard({ user }: UserCardProps) {
  ...
  return (
    <div>
      ...
      {propStatus.user?.profile?.bio?.isSuccess && <p>{user.profile.bio}</p>}
    </div>
  );
}
```
...
For array props, access `completedItems` and `streamingItems` to show partial results:
...
function TaskList({ tasks }: TaskListProps) {
  ...
  return (
    <div>
      ...
      {propStatus.tasks?.streamingItems?.map((task) => (
        <TaskCard key={task.id} {...task} isStreaming />
      ))}
    </div>
  );
}
...
| `propStatus`   | `DeepPropStatus<Props>`                                    | Per-prop streaming status with nested tracking. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
