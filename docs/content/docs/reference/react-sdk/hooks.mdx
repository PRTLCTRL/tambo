...
For props with nested objects, `propStatus` contains nested status objects that track the streaming state of each nested field:
...
function UserCard(props: UserCardProps) {
  ...
  // Access nested field status
  const userStatus = propStatus.user as PropStatus & {
    name?: PropStatus;
    email?: PropStatus;
    profile?: PropStatus & { bio?: PropStatus };
  };

  return (
    <div>
      <h3 className={userStatus.name?.isStreaming ? "animate-pulse" : ""}>
        {props.user.name}
      </h3>
      <p className={userStatus.email?.isStreaming ? "animate-pulse" : ""}>
        {props.user.email}
      </p>
      {userStatus.profile?.bio?.isStreaming && <Spinner />}
    </div>
  );
}
```
...
For array props, `propStatus` includes `completedItems` and `streamingItems` arrays:
...
function StreamingList(props: ListProps) {
  const { propStatus } = useTamboStreamStatus<ListProps>();

  return (
    <ul>
      {/* Render completed items */}
      {propStatus.items?.completedItems?.map((item, idx) => (
        <li key={idx}>{item}</li>
      ))}

      {/* Render streaming items with loading indicator */}
      {propStatus.items?.streamingItems?.map((item, idx) => (
        <li key={`streaming-${idx}`} className="animate-pulse">
          {item}
        </li>
      ))}
    </ul>
...
This pattern is useful when streaming lists of items - you can show completed items immediately while indicating which items are still being generated.
