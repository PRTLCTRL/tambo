...
function ProfileCard(props: UserProfile) {
  const { propStatus } = useTamboStreamStatus<UserProfile>();

  return (
    <div>
      <h3 className={propStatus.user?.name?.isStreaming ? "loading" : ""}>
        {props.user?.name}
      </h3>
      <p className={propStatus.user?.email?.isStreaming ? "loading" : ""}>
        {props.user?.email}
      </p>
      <p className={propStatus.user?.address?.city?.isStreaming ? "loading" : ""}>
...
For array props, use `completedItems` and `streamingItems` to show only completed items or indicate streaming progress:
...
function ItemGrid(props: ItemList) {
  const { propStatus } = useTamboStreamStatus<ItemList>();

  const completedItems = (propStatus.items?.completedItems ?? []) as typeof props.items;
  const streamingItems = (propStatus.items?.streamingItems ?? []) as typeof props.items;
    ...
    <div>
      ...
      {streamingItems.length > 0 && (
        <div className="streaming-indicator">
          Streaming {streamingItems.length} items...
        </div>
      )}
    </div>
  );
...
This pattern is useful when you want to display completed items immediately while the AI continues streaming more items.
