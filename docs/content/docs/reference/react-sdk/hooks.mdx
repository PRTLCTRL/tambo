...
function TaskList(props: TaskListProps) {
  ...
  return (
    <div>
      ...
      {propStatus.tasks?.streamingItems && propStatus.tasks.streamingItems.length > 0 && (
        <div className="opacity-50">
          <p>Streaming {propStatus.tasks.streamingItems.length} more items...</p>
        </div>
      )}
    </div>
  );
...
<Callout type="tip">
  ...
  For arrays, use `propStatus.arrayField?.completedItems` to render only finished items.
</Callout>
