...
Access streaming status for nested properties using dot notation:
...
function UserCard(props: UserProps) {
  ...
  return (
    <div>
      ...
      <p className={propStatus.user?.profile?.bio?.isStreaming ? "animate-pulse" : ""}>
...
For array props, access `completedItems` and `streamingItems` to render items as they complete:
...
function ItemList(props: ListProps) {
  ...
  const completed = propStatus.items?.completedItems as ListProps["items"] ?? [];
  const streaming = propStatus.items?.streamingItems as ListProps["items"] ?? [];
  ...
  return (
    <div>
      ...
      {streaming.map((item) => (
        <div key={item.id} className="opacity-50 animate-pulse">
          <h3>{item.name}</h3>
          <p>{item.description}</p>
        </div>
      ))}
