...
### Example: Nested Object Tracking

Track the streaming status of properties within nested objects:
...
### Example: Array Item Tracking

Display completed items from an array as they finish streaming:
...
function TaskList({ tasks }: TaskListProps) {
  ...
  const streamingItems = propStatus.tasks?.streamingItems || [];
    ...
    <div>
      ...
      {streamingItems.length > 0 && (
        <>
          <h3>Loading Tasks...</h3>
          <ul>
            {streamingItems.map((task, index) => (
              <li key={task.id || index} className="animate-pulse">
                {task.title || "Loading..."}
              </li>
            ))}
          </ul>
