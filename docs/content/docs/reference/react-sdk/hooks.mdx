...
Track streaming status for components rendered via `ComponentRenderer`. Provides both overall stream status and per-prop granularity. Supports nested objects and array tracking.
...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus<Props[keyof Props]>>>`                | Per-prop streaming status with nested object and array support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus) and [`ArrayPropStatus`](/reference/react-sdk/types#arraypropstatus). |

### Nested Object Tracking

Access status for nested properties using dot notation:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      ...
      <div>
        {propStatus.user?.address?.city?.isSuccess && (
...
For array props, access `completedItems` and `streamingItems` to show only completed items or handle partial arrays:
    ...
    <div>
      ...
      {streamingTodos.length > 0 && (
        <div>
          <h2>Streaming...</h2>
          <ul>
            {streamingTodos.map((todo: any, i) => (
              <li key={i} className="animate-pulse">{todo.title}</li>
            ))}
          </ul>
        </div>
      )}
