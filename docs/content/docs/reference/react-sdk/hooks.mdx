...
Track streaming status for components rendered via `ComponentRenderer`. Provides both overall stream status and per-prop granularity, with support for nested objects and arrays.
...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus>>`          | Per-prop streaming status with nested object and array support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
...
For props with nested objects, `propStatus` recursively tracks the streaming status of each nested property. Access nested status using dot notation:
...
function UserCard(props: UserCardProps) {
  ...
  // Check if entire user object is streaming
  if (propStatus.user?.isStreaming) {
    return <Skeleton />;
  }

  // Show skeleton for individual fields that are still streaming
  return (
    <div>
...
Each nested property has its own `PropStatus` with `isPending`, `isStreaming`, `isSuccess`, and `error` fields.

### Array Support

For array props, `propStatus` includes `completedItems` and `streamingItems` arrays that let you render completed items while the rest stream:
...
function ItemList(props: ItemListProps) {
  const { propStatus } = useTamboStreamStatus<ItemListProps>();

  // Show completed items immediately
  ...
  return (
    <div>
      ...
      {propStatus.items?.isPending && <Skeleton />}
    </div>
  );
}
```
...
- `completedItems`: Array items that have finished streaming
- `streamingItems`: Array items currently streaming
- Standard status fields (`isPending`, `isStreaming`, `isSuccess`, `error`)
