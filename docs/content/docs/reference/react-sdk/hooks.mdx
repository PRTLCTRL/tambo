...
For props with nested object structures, `propStatus` contains nested status objects that mirror the prop structure:
...
function UserCard(props: UserProps) {
  ...
  return (
    <div>
      ...
      <div
        className={propStatus.user?.profile?.bio?.isStreaming ? "loading" : ""}
      >
...
For array props, `propStatus` includes `completedItems` and `streamingItems` arrays to track which items have finished streaming:
