...
**Nested Objects**: The `propStatus` object mirrors your prop structure. Access nested properties like `propStatus.user?.name?.isStreaming`.

**Arrays**: Array props include `completedItems` and `streamingItems` arrays, allowing you to display completed items while streaming continues.
...
### Example: Nested Object Status

Track the streaming status of nested properties:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      <h2 className={propStatus.user?.name?.isStreaming ? "loading" : ""}>
        {props.user.name}
      </h2>

      <p className={propStatus.user?.email?.isStreaming ? "loading" : ""}>
        {props.user.email}
      </p>

      <div className={propStatus.user?.profile?.bio?.isStreaming ? "loading" : ""}>
        {props.user.profile.bio}
      </div>

      {propStatus.user?.profile?.avatar?.isSuccess && (
        <img src={props.user.profile.avatar} alt="Profile" />
      )}
    </div>
  );
}
...
function TaskList(props: TaskListProps) {
  ...
  // Get completed and streaming items
  const completedTasks = (propStatus.tasks?.completedItems || []) as TaskListProps["tasks"];
  const streamingTasks = (propStatus.tasks?.streamingItems || []) as TaskListProps["tasks"];
    ...
    <div>
      ...
      <ul>
        ...
        {streamingTasks.map((task, i) => (
          <li key={`streaming-${i}`} className="animate-pulse">
            {task.title}...
          </li>
        ))}
      </ul>
      {propStatus.tasks?.isSuccess && (
        <p>All tasks loaded ({completedTasks.length} total)</p>
      )}
    </div>
  );
}
...
### Example: Mixed Nested and Array

Handle complex prop structures with nested objects and arrays:
...
function Dashboard(props: DashboardProps) {
  const { streamStatus, propStatus } = useTamboStreamStatus<DashboardProps>();

  const userStatus = propStatus.user as PropStatus & {
    name?: PropStatus;
    notifications?: PropStatus;
  };

  const completedNotifications =
    (userStatus?.notifications?.completedItems as string[]) || [];

  return (
    <div>
      {/* Show user name with loading state */}
      <h1 className={userStatus?.name?.isStreaming ? "loading" : ""}>
        {props.user.name}
      </h1>
      ...
      <div>
        <h2>Notifications</h2>
        <ul>
          {completedNotifications.map((notification, i) => (
      ...
      {propStatus.stats?.isSuccess && (
        <div>
          <p>Views: {props.stats.views}</p>
          <p>Likes: {props.stats.likes}</p>
        </div>
      )}
    </div>
  );
