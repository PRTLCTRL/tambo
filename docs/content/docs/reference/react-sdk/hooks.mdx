...
Track streaming status of nested object properties by accessing nested fields in `propStatus`:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      ...
      {propStatus.user?.email?.isSuccess && <p>{props.user.email}</p>}
...
For array props, access `completedItems` and `streamingItems` to render only the items that have finished streaming:
...
function RecipeList(props: RecipeListProps) {
  ...
  const completed = propStatus.recipes?.completedItems || [];
  const streaming = propStatus.recipes?.streamingItems || [];
  ...
  return (
    <div>
      ...
      {streaming.length > 0 && <div>Loading {streaming.length} more...</div>}
    </div>
  );
}
```
