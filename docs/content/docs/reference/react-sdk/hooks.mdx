...
| `propStatus`   | `Partial<Record<keyof Props, PropStatus>>`                | Per-prop streaming status with nested support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
...
For components with nested object props, the status structure mirrors the prop structure. Access nested field status using dot notation:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      <h2 className={propStatus.user?.name?.isStreaming ? "shimmer" : ""}>
        {props.user.name}
      </h2>
      <p className={propStatus.user?.email?.isStreaming ? "shimmer" : ""}>
        {props.user.email}
      </p>
...
Each nested object has its own status flags (`isPending`, `isStreaming`, `isSuccess`, `error`) plus nested status for its child properties.

### Array Status with Completed Items

For array props, the status includes `completedItems` and `streamingItems` arrays. This lets you render only the items that have finished streaming:
...
function TaskList(props: TaskListProps) {
  ...
  const streamingTasks = propStatus.tasks?.streamingItems ?? [];

  return (
    <div>
      <h2>Tasks ({completedTasks.length} ready)</h2>
      <ul>
      ...
      {streamingTasks.length > 0 && (
        <div className="streaming-section">
          <p>Loading {streamingTasks.length} more tasks...</p>
        </div>
      )}
    </div>
  );
...
While the array is streaming, items appear in `streamingItems`. Once streaming completes, all items move to `completedItems`.
