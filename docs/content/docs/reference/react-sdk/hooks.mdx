...
The hook now supports tracking nested object properties and array items:

- **Nested objects**: Access nested property status using dot notation: `propStatus.user.name.isStreaming`
- **Arrays**: Access `completedItems` and `streamingItems` to show only completed items during streaming
