...
Track streaming status for nested object properties by accessing them through the status object:
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      {/* Access nested property status using dot notation */}
...
Track completed and streaming items in array props using `completedItems` and `streamingItems`:
...
function TaskList(props: TaskListProps) {
  ...
  // Only display completed items during streaming
  const displayTasks =
    propStatus.tasks?.completedItems || props.tasks || [];
  ...
  return (
    <ul>
      ...
      {propStatus.tasks?.streamingItems?.length > 0 && (
        <li className="opacity-50 animate-pulse">Loading more...</li>
      )}
    </ul>
  );
}
...
| `propStatus`   | `Partial<Record<keyof Props, PropStatus>>`                | Per-prop streaming status. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). Supports nested access for objects and includes `completedItems`/`streamingItems` for arrays. |
