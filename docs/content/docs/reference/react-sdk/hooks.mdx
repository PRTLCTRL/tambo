...
| `propStatus`   | `Partial<Record<keyof Props, NestedPropStatus>>`                | Per-prop streaming status with nested support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |
...
When your component has nested object properties, `useTamboStreamStatus` tracks the streaming status at each level of nesting. This allows you to show loading indicators or apply styles to specific nested fields.
...
function UserCard(props: UserCardProps) {
  ...
  return (
    <div>
      ...
      {propStatus.user?.email?.isSuccess ? (
        <a href={`mailto:${props.user.email}`}>{props.user.email}</a>
      ) : (
        <Skeleton />
      )}

      {propStatus.user?.profile?.bio?.isStreaming && (
...
For array properties, `useTamboStreamStatus` provides `completedItems` and `streamingItems` arrays, allowing you to render items as they complete streaming rather than waiting for the entire array.
...
function TodoList(props: TodoListProps) {
  const { propStatus } = useTamboStreamStatus<TodoListProps>();

  const completedTodos = propStatus.todos?.completedItems ?? [];
  const streamingTodos = propStatus.todos?.streamingItems ?? [];
    ...
    <div>
      ...
      {streamingTodos.length > 0 && (
        <div>
          <h2>Loading...</h2>
          <ul>
            {streamingTodos.map((todo) => (
              <li key={todo.id} className="animate-pulse">
                {todo.title}
              </li>
            ))}
          </ul>
      ...
      {propStatus.todos?.isPending && <div>Waiting for tasks...</div>}
    </div>
  );
}
```
...
**Progressive Disclosure**: Show completed array items immediately while streaming continues:
...
**Nested Field Loading States**: Apply loading indicators to specific nested fields:
...
return (
  <div>
    {propStatus.user?.profile?.avatar?.isStreaming ? (
      <Skeleton className="w-16 h-16 rounded-full" />
    ) : (
      <img src={props.user.profile.avatar} alt="Avatar" />
    )}
  </div>
);
```

**Wait for Critical Fields**: Only render when specific nested fields are complete:

```tsx
const { propStatus } = useTamboStreamStatus<Props>();

if (!propStatus.config?.apiKey?.isSuccess) {
  return <div>Waiting for configuration...</div>;
}

return <ConfiguredComponent apiKey={props.config.apiKey} />;
```
