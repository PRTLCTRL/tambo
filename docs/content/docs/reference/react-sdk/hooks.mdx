...
Track streaming status for nested properties within object props:
...
Track completed and streaming items in array props:
...
function ItemList(props: ListProps) {
  ...
  const streamingItems = propStatus.items?.streamingItems || [];
      ...
      {streamingItems.length > 0 && (
        <div>
          ...
          {streamingItems.map(item => (
            <div key={item.id} className="animate-pulse">{item.title}</div>
          ))}
        </div>
      )}
    </div>
...
| `propStatus`   | `Partial<Record<keyof Props, PropStatus>>`                | Per-prop streaming status with nested object support. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk/types#propstatus). |

For nested objects, `propStatus` contains nested status objects matching your prop structure. For arrays, the status includes `completedItems` and `streamingItems` arrays containing the actual items at each streaming stage.
