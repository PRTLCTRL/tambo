...
For props with nested object structures, `propStatus` provides status tracking for each nested property:
...
function UserCard(props: UserCardProps) {
  ...
  const userStatus = propStatus.user;
  ...
  return (
    <div>
      ...
      <address>
        <div
          className={
            userStatus?.address?.street?.isStreaming ? "animate-pulse" : ""
          }
        >
        ...
        <div
          className={
            userStatus?.address?.city?.isStreaming ? "animate-pulse" : ""
          }
        >
...
Each nested property has its own `PropStatus` with `isPending`, `isStreaming`, `isSuccess`, and `error` flags.

### Array Tracking

For array props, `propStatus` provides `completedItems` and `streamingItems` to differentiate between items that have finished streaming and those still in progress:
...
function TaskList(props: TaskListProps) {
  ...
  const tasksStatus = propStatus.tasks;

  return (
    <div>
      {tasksStatus?.completedItems && tasksStatus.completedItems.length > 0 && (
        <div>
          <h3>Completed Tasks</h3>
          <ul>
            {(tasksStatus.completedItems as TaskListProps["tasks"]).map(
              (task) => (
      ...
      {tasksStatus?.streamingItems && tasksStatus.streamingItems.length > 0 && (
        <div>
          <h3>Loading Tasks</h3>
          <ul>
            {(tasksStatus.streamingItems as TaskListProps["tasks"]).map(
              (task) => (
                <li key={task.id} className="animate-pulse">
                  {task.title}
                </li>
              ),
            )}
...
The `completedItems` array contains items that have finished streaming, while `streamingItems` contains items currently being streamed. When streaming completes, all items move to `completedItems`.
