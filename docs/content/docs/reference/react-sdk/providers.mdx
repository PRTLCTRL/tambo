...
| `autoAddComponentsToInteractables` | `boolean`                                        | No       | Automatically add AI-generated components to interactables for updates in future messages (default: `false`) |
...
### Example with Automatic Interactables

Enable automatic interactables mode to make all AI-generated components available for updates:

```tsx
<TamboProvider
  apiKey={process.env.NEXT_PUBLIC_TAMBO_API_KEY}
  userKey={session.user.id}
  components={[WeatherCard, StockChart, TodoList]}
...
When enabled, any component the AI generates is automatically added to the interactables list. Users can then iteratively refine components:

1. User: "Show me a weather card for San Francisco"
2. AI generates `WeatherCard` → automatically added to interactables
3. User: "Change that to show Seattle instead"
4. AI updates the existing `WeatherCard` component

See [Interactable Components](/concepts/generative-interfaces/interactable-components) for more details.
