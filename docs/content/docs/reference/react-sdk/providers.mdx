...
| `autoInteractables`         | `boolean`                                        | No       | Automatically add generated components to interactables list (default: `false`). See Automatic Interactables below. |
...
When `autoInteractables` is set to `true`, Tambo automatically adds all AI-generated components to the interactables list. This allows users to modify these components through natural language conversation without manually wrapping them with `withTamboInteractable`.
...
With this enabled:
- Any component Tambo generates in response to user messages becomes automatically interactable
- Users can ask Tambo to modify generated components (e.g., "update that weather card to show Tokyo")
- The component registry must include all components that might be generated (via the `components` prop)
- Each component must have a `propsSchema` defined for Tambo to know which props can be updated

This is ideal for chat-style interfaces where the component list is dynamic and you want all generated components to be editable through conversation.

For more control over which components are interactable, use manual interactables by wrapping specific components with `withTamboInteractable` instead. See the [Interactable Components guide](/guides/enable-generative-ui/register-interactables) for details.
