...
interface PropStatus {
  ...
  completedItems?: unknown[]; // For array props: completed items
  streamingItems?: unknown[]; // For array props: all items (including incomplete)
}
```

For nested object props, `PropStatus` includes nested status properties matching the object structure. For example, if your component has a `user` prop with `name` and `email` fields, `propStatus.user` will include `name` and `email` status objects.
